...
4. **The production default settles honestly and silently.** With no `?net` param: within ~15 s of `data-loaded="true"`, `data-net` reads exactly `"webrtc"` or `"off"` (never absent, blank, or intermediate), and over a further ~30 s there are zero console errors and zero unhandled promise rejections, including when relays/WebRTC are unreachable (at most one info-level message). Either way the solo room behaves identically to sprint 12 (`data-peers="0"`, walk, focus, Enter, Escape all work).
